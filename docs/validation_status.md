...
The first post-equilibration curvature-only production-baseline ensembles stopped near 120 grains and reproducibly returned an apparent early-time exponent near one. Historical filenames retain the string `baseline_qiu`, but these results are not a reproduction of `QIU_SI_REFERENCE`. Expanding from 5 to 20 realizations did not remove that result, so it is preserved in `baseline_qiu_ensemble20_n1_failure.json`. Exact checkpoint continuation then extended all 20 runs to 60--61 grains without changing their earlier histories.
