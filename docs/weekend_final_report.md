...
| Q23 | At what temperature does the dominant difficult event cross from shear/TJ compatibility to climb/point-defect control? | FP-C2 now supplies the climb reference: above 900 K its local growth/event slopes stabilize near 0.74--0.76 eV, whereas its 800--900 K slopes are 0.572/0.514 eV. Locating a coupled-process crossover still requires FP-C3--FP-C5 and FP-SC3. |
| Q24 | Does that crossover curve the Arrhenius plot or change the growth exponent? | FP-G2, FP-T2, FP-S2, and FP-C2 all retain common `n=1` while their local Arrhenius slopes change strongly. FP-C2 growth/event slopes change 0.572→0.743→0.739 and 0.514→0.746→0.761 eV; FP-S2 slopes rise 0.443→0.564 and 0.336→0.638 eV. Coupled physics curves Arrhenius behavior without changing the fitted common exponent. |
