...
def main():
    ...
    if manufactured.get('cases'):
        ...
        for name in ('A0_ISOTROPIC', 'A2_STRONG'):
            ...
            if points:
                timesteps.update(v[0] for v in points)
                axes[0].loglog([v[0] for v in points], [v[1] for v in points], 'o-', label=name)
                finest_energy = points[0][2]
                axes[1].semilogx([v[0] for v in points], [abs(v[2]-finest_energy)/finest_energy for v in points], 'o-', label=name)
        axes[0].axhline(.01, color='black', ls='--', label='1% balance gate')
        axes[0].set_ylabel('Maximum relative\ndissipation-balance error')
        axes[1].set_ylabel('Relative final energy difference\nfrom finest dt')
        for ax in axes:
            ax.set_xticks(sorted(timesteps), [f'{dt:g}' for dt in sorted(timesteps)], fontsize=9)
            ax.minorticks_off()
            ax.set_xlabel('Requested timestep')
            ax.grid(alpha=.2)
            ax.legend(fontsize=8)
        fig.suptitle('Sharp-network time refinement — no PF qualification', fontsize=11)
        fig.savefig(dst/'sharp_network_timestep.png', dpi=160, bbox_inches='tight')
        plt.close(fig)
    ...
    dump(dst/'report_provenance.json', {
        'generator': Path(__file__).name,
        'generator_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        'retrieved_output': str(src.resolve()),
        'earlier_outputs': [str(path.resolve()) for path in args.earlier_output],
        'python_version': platform.python_version(),
        'matplotlib_version': matplotlib.__version__ if report.get('ladder') or manufactured.get('cases') else None,
        'scientific_evolution_or_constitutive_reevaluation': False,
        'operations': 'compact JSON/XML evidence assembly and lightweight summary plotting',
    })
