#!/usr/bin/env python3
"""Build the checkpointed pristine Qiu-SI preflight for the historical runtime."""
...
def instrument(source: str) -> str:
    ...
    source = replace_once(
        ...
        "from functions_4ref_new import *\nimport sparse\n"
        "from qiu_native_preflight_v3_instrumentation import (CHECKPOINT_STEPS, boundary_pairs, morphology, write_checkpoint)",
    )
...
def emit_preflight_checkpoint(accepted_step, pre_delta, accepted_delta):
    ...
    # Output-only instrumentation must never advance the live stress history.
    diagnostic_sigma11_R1=sigma11_R1.copy(); diagnostic_sigma12_R1=sigma12_R1.copy(); diagnostic_sigma22_R1=sigma22_R1.copy()
    diagnostic_sigma11_R2=sigma11_R2.copy(); diagnostic_sigma12_R2=sigma12_R2.copy(); diagnostic_sigma22_R2=sigma22_R2.copy()
    diagnostic_gb_x, diagnostic_gb_y = find_gb(phi, nf_diagnostic, mf_diagnostic, nx, ny, dx, dy, number_of_grain, G, diagnostic_sigma11, diagnostic_sigma12, diagnostic_sigma22, diagnostic_sigma11_b, diagnostic_sigma12_b, diagnostic_sigma22_b, sigma12_ext, sigma11_ext, sigma22_ext, ddd, accepted_step, stress_step, diagnostic_sigma11_R1, diagnostic_sigma12_R1, diagnostic_sigma22_R1, diagnostic_sigma11_R2, diagnostic_sigma12_R2, diagnostic_sigma22_R2, misorientation_list, ref_theta_i, diagnostic_pairs2, diagnostic_pairs3)
...
_manifest["instrumentation"]="qiu-native-preflight-v3"
...
print("QIU_NATIVE_HISTORICAL_OBJECT_MODE", ",".join(_object_mode))
print("QIU_NATIVE_PREFLIGHT_V3_SEGMENT_COMPLETE", nsteps)
'''
    return source
...
def build(source_dir: Path, instrumentation: Path, output: Path, source_variant: str) -> dict:
    ...
    generated = output / "Bicrystal-4reference-el-pf-preflight-v3.py"
    ...
    manifest = {
        "schema": "qiu-native-preflight-v3-build",
...
def main() -> None:
    ...
    parser.add_argument("--source-variant", choices=sorted(FUNCTION_VARIANTS), default="pristine_archive")
