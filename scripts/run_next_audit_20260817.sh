...
BASE_CAMPAIGN=$(python scripts/run_campaign.py \
...
python scripts/analyze_campaign.py "$BASE_CAMPAIGN" \
  --output results/production_summaries/baseline_reconciliation_20_summary.csv
python scripts/plot_campaign.py "$BASE_CAMPAIGN" \
...
python scripts/audit_barrierless_fraction.py "$CLIMB_CAMPAIGN" \
...
if [[ -n "${MECHANISM_CAMPAIGN:-}" ]]; then
  ...
  python scripts/audit_barrierless_fraction.py "$MECHANISM_CAMPAIGN" \
