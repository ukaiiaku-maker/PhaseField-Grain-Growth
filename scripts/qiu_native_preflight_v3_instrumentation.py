"""Numeric checkpoint instrumentation for the historical-runtime native Qiu-SI preflight."""
...
CHECKPOINT_STEPS = (0, 1, 10, 100, 500, 1000)
...
def write_checkpoint(output, step, dt, phi, orientations, eij, pre_delta, post_delta, sigma11, sigma12, sigma22, gb_x, gb_y, boundary_pairs, ref_theta_i, nf_diag, mf_diag, dx, dy, eta, beta_function, initial_population=None, native_trial_buffer=None, trajectory_state=None):
    ...
    manifest_path=output/'checkpoint_manifest.json';manifest=json.loads(manifest_path.read_text()) if manifest_path.exists() else {'schema':'qiu-native-preflight-v3-checkpoints','checkpoints':[]}
